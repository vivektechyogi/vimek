...
import { Menu, Home, BookOpen, CalendarCheck, Send, Edit3, Users } from 'lucide-react';
...
export function Navbar() {
  ...
  const handleScroll = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    } else if (href === '/' && pathname ==='/') { // Special case for home link when already on homepage
        e.preventDefault();
        ...
        <Link
          ...
          className={cn(
            'text-sm font-medium transition-colors hover:text-primary',
             // Active state logic for single page:
             // Check if current hash matches link's hash.
             // For '/', ensure no hash or it's '#hero'.
            typeof window !== 'undefined' && window.location.hash === link.href.substring(1) && link.href.startsWith('/#') || (link.href === '/#hero' && (pathname === '/' && (window.location.hash === '' || window.location.hash === '#hero')))
